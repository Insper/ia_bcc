...
\begin{Slide}{\textbf{Problema das N rainhas}}
		...
		Dado um tabuleiro $N \times N$, é possível encontrar uma configuração onde $N$ rainhas no tabuleiro não conseguem atacar nenhuma das outras rainhas no mesmo tabuleiro?
...
\begin{Slide}{Introdução}
	...
	No entanto, existem outras classes de problemas onde o objetivo não está em encontrar uma sequência de ações, mas sim \textbf{encontrar um ou mais estados que satisfazem um conjunto de restrições}.
...
\begin{Slide}{\textbf{Análise} do algoritmo BSM}
  \begin{itemize}
...
\begin{Slide}{Algoritmo \textbf{Subida da Montanha Estocástico}}
{\footnotesize
\begin{algorithmic} 
...
\begin{Slide}{\textbf{Análise} do algoritmo BSM Estocástico}
  \begin{itemize}
  \item Completo:  \textbf{sim} (se a probabilidade para geração de estados iniciais for equiprovável)
  \item Ótimo:  não se aplica
  \item Tempo:  \textbf{?}
  \item Espaço:  \textbf{nada}!
  \end{itemize}
...
\begin{Slide}{Material de \textbf{consulta}}
  \begin{itemize}
  \item Capítulo 6 da $3^{a}$ edição do livro do Russell \& Norvig
  \end{itemize}
\end{Slide}
