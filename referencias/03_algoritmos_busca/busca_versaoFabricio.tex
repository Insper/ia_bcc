...
\begin{Slide}{\textbf{Análise} do algoritmo BL}
  ...
  \item Tempo: explorar $O(b^d)$\footnote{A notação $O(b^d)$ representa uma ordem de crescimento assintótica. Não representa um valor exato, mas sim um limite superior para o crescimento da função.} nodos
...
\begin{Slide}{\textbf{Estratégias} de poda da árvore de busca}
	...
	Poda da árvore na biblioteca \textsc{aigyminsper}: 
...
# Não permite criar um filho igual ao pai
algorithm = BuscaLargura()
result = algorithm.search(
	state, pruning='father-son')

# Não cria um nodo que já existe na árvore
algorithm = BuscaLargura()
result = algorithm.search(
